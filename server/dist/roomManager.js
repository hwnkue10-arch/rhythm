...
class RoomManager {
    ...
    joinRoom(roomId, socket, nickname) {
        ...
        if (runtime.state.phase !== "lobby")
            return { error: "이미 게임이 진행 중인 방입니다." };
        if (Object.keys(runtime.state.players).length >= 4)
    ...
    makePlayer(id, nickname, colorIndex) {
        return {
            id,
            nickname,
    ...
    broadcast(roomId, payload, exceptPlayerId) {
        const runtime = this.rooms.get(roomId);
        if (!runtime)
            return;
    ...
    setSong(roomId, playerId, slot, song) {
        const runtime = this.rooms.get(roomId);
        if (!runtime || runtime.state.hostId !== playerId)
            return;
        if (runtime.state.phase !== "lobby")
            return;
    ...
    kick(roomId, hostId, targetId) {
        const runtime = this.rooms.get(roomId);
        if (!runtime || runtime.state.hostId !== hostId || hostId === targetId)
            return;
        const sock = runtime.sockets.get(targetId);
        delete runtime.state.players[targetId];
        runtime.sockets.delete(targetId);
        if (sock && sock.readyState === ws_1.WebSocket.OPEN) {
            sock.send(JSON.stringify({ type: "kicked" }));
            sock.close();
        }
        this.broadcastRoomState(roomId);
    }
    transferHost(roomId, hostId, targetId) {
        const runtime = this.rooms.get(roomId);
        if (!runtime || runtime.state.hostId !== hostId)
            return;
        if (!runtime.state.players[targetId])
            return;
        runtime.state.hostId = targetId;
        this.broadcastRoomState(roomId);
    }
    updatePosition(roomId, playerId, x, y) {
        const runtime = this.rooms.get(roomId);
        if (!runtime)
            return;
     ...
     */
    beginStage(roomId, stage, timeline, onStageEnd) {
        const runtime = this.rooms.get(roomId);
        if (!runtime)
            return;
        ...
        this.broadcastRoomState(roomId);
        if (runtime.stageEndTimer)
            clearTimeout(runtime.stageEndTimer);
    ...
    handleHit(roomId, playerId, onAllDead) {
        ...
        if (allDead) {
            if (runtime.failCheckTimer)
                clearTimeout(runtime.failCheckTimer);
    ...
    handleRevive(roomId, reviverId, targetId) {
        const runtime = this.rooms.get(roomId);
        if (!runtime || runtime.state.phase !== "playing")
            return false;
        const reviver = runtime.state.players[reviverId];
        const target = runtime.state.players[targetId];
        if (!reviver || !target || reviver.dead || !target.dead || !target.diedAt)
            return false;
        const now = Date.now();
        // 1. 사망 직후 다운 상태(0.7초)에는 즉시 부활 불가 -> 겹침 무한 무적 루프 방지!
        if (now - target.diedAt < REVIVE_DOWN_TIME_MS)
            return false;
        // 2. 3초 경과 시 부활 기회 상실
        if (now - target.diedAt > REVIVE_WINDOW_MS)
            return false;
    ...
    restartStage(roomId, hostId, onStageEnd) {
        const runtime = this.rooms.get(roomId);
        if (!runtime || runtime.state.hostId !== hostId || runtime.state.phase !== "stage_failed")
            return;
    ...
    advanceToNextStage(roomId, onStageEnd, timeline) {
        const runtime = this.rooms.get(roomId);
        if (!runtime)
            return;
