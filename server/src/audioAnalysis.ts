...
import { ObstacleEvent, Timeline, InstrumentType } from "./types";
...
export function detectInstrumentOnsets(samples: Float32Array): InstrumentOnset[] {
  ...
  const instKeys = Object.keys(INSTRUMENT_RANGES) as (keyof typeof INSTRUMENT_RANGES)[];

  const binRanges: Record<string, [number, number]> = {};
  ...
  const energyMap: Record<string, number[]> = { kick: [], snare: [], melody: [], hihat: [] };
  ...
  const gapFrames: Record<string, number> = {
  ...
  const totFlux = [0];
  for (let i = 1; i < normTot.length; i++) totFlux.push(Math.max(0, normTot[i] - normTot[i - 1]));
 ...
 */
function makeInstrumentEvent(onset: InstrumentOnset, stage: number, rand: () => number, melodyIndex: { v: number }): ObstacleEvent {
  ...
  if (instrument === "kick") {
    ...
    const sweepRanges: Record<string, { start: number; speed: number }> = {
 ...
 */
export function buildTimeline(onsets: InstrumentOnset[], songId: string, stage: number, durationSec: number): Timeline {
